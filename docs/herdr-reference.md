...
## 三之二、`prefix+g` 的导航器——**herdr 里「看到活跃 agents」的主入口**

```
 spaces                  │ 1       +
                         │
 · ~                     │# gejiliang @ mini in ~ [12:10:47]
         ┌──────────────────────────────────────────────────────────────────────┐
         │ / search panes                                                     1 │
         │──────────────────────────────────────────────────────────────────────│
         │ ◆ ▾ · ~ (1)                                                          │
...
一个**居中浮层**：顶部 ` / search panes` 加右侧计数，一条横线，下面是
**space → pane 的树**，每行右列写着这个 pane 在跑什么（`shell`，跑 agent 时应是 agent 名）。

**这一点我原来理解错了**：我把「活跃 agent」做成了常驻挂在侧栏每个工作区下面的列表；
herdr 的主入口是这个**可搜索的浮层**，侧栏那个 `agents` 格是另一个东西。

dshr 这边尤其该有它——一个 pane 就是一个 agent 会话，
所以这棵树天然就是「所有活跃 agent + 它们的状态」，而且状态来自 host 权威事件。

> 状态：**还没做**。当前这一轮先对齐外壳（第一、二、四、五节），导航器是下一步。
