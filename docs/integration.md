...
> ⚠️ **`@dshr/shell` 已经删掉了**（2026-08-16，方向性错误：那是在 herdr 旁边复刻 herdr）。
> 本文原本是「shell 的占位 props 与 tui 真实 props 对不上」的裁决书，那部分现在只有考古价值。
>
> **仍然成立、且**别再踩**的是这几条**：
>
> - **第 6 节**：ink 的 `<Static>` 是文档级的，输出永远写在动态区上方，
>   **关不进任何容器**。当年是被多 pane 暴露的；现在虽然只有一个 pane，
>   但会话视图旁边还有右侧信息列，用了照样跑出去。所以 `Conversation` 一直不用 `<Static>`。
> - **第 2 节的 `disabled` / `acceptsKey`**：输入框不能靠「渲染时的值」判活，
>   要在**按键到达那一刻**现问一次，否则前缀动作之后紧接着的键会被永久丢掉。
> - **第 5 节**：`@dshr/state` 的工厂名是 `createState`。
>
> 下面的原文保留，读的时候记住 shell 那一层已经不存在了。
