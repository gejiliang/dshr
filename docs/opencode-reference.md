...
**已实现并逐字核对过两轮**（2026-08-16）：大结构（竖线输入框、无竖线助手消息、
`▣` 页脚、右侧信息列、底部栏、ASCII logo）在第一轮合入；第二轮对着上游源码与
带色截屏修了字形与颜色（真箭头 `→`、agent 色 secondary、panel 底色整行涂满、
`▣` 后两空格等，详见 [`opencode-port.md`](opencode-port.md) 第七节）。
本文剩下的价值是「opencode 长这样」的实测底档；数值以 port 文档与源码为准。
